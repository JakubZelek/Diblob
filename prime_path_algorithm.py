...
class SCC:
    ...
    def __init__(self, scc_graph: dict, graph: dict):
        ...
        self.scc_entry_nodes = {
            ...
            if any(
                scc_node_id in graph[node_id] for node_id in graph if node_id not in scc_graph
            )
        }
        self.scc_exit_nodes = {
            scc_node_id
            for scc_node_id in scc_graph
            if any(
                node_id in graph[scc_node_id] for node_id in graph if node_id not in scc_graph
            )
        }
        ...
        for scc_entry in self.scc_entry_nodes:
            for scc_exit in self.scc_exit_nodes:
                scc_entry_exit_list.extend(
                    list(scc_entry_exit_paths_extraction(self.internal_paths, scc_entry, scc_exit))
                )
...
def scc_entry_exit_paths_extraction(scc_internal_prime_paths: set[Path], v_en: str, v_ex: str):
    """
    Algorithm 2 from [1]
    """
...
def check_outgoing(outgoing: str, q: Path, scc_dict: dict[str, SCC], graph: dict):
    ...
    match outgoing:
        case _ if outgoing.startswith("SCC"):
            return any(
                node_id in graph[q[-1]] for node_id in scc_dict[outgoing].scc_entry_nodes
            )
        case _:
...
def cfg_complete_prime_paths_generation(prime_paths_of_ccfg: list[Path], scc_dict: dict[str, SCC], graph: dict):
    """
    Algorithm 5 from [1]
    """
...
def scc_exit_prime_paths_generation(complete_prime_paths: set[Path], scc_dict: dict[str, SCC]):
    """
    Algorithm 6 from [1]
    """
    result = set()
...
def scc_entry_prime_paths_generation(
    complete_prime_paths: set[Path], scc_exit_prime_paths: set[Path], scc_dict: dict[str, SCC]
):
    """
    Algorithm 7 from [1]
    """
...
def get_internal_prime_paths(scc_dict: dict[str, SCC]):
    ...
    for scc in scc_dict.values():
        boundary = scc.entry_paths | scc.exit_paths
...
def get_prime_paths(graph: dict):
    """
    ...
    for node_id in ccfg.keys():
        if all(node_id not in outgoing for outgoing in ccfg.values()):
            ...
            complete_prime_paths = cfg_complete_prime_paths_generation(non_empty_prime_paths, scc_dict, graph)
            for complete_prime_path in complete_prime_paths:
                yield complete_prime_path

            scc_exit_prime_paths = scc_exit_prime_paths_generation(complete_prime_paths, scc_dict)
            for scc_exit_prime_path in scc_exit_prime_paths:
                yield scc_exit_prime_path

            scc_entry_prime_paths = scc_entry_prime_paths_generation(complete_prime_paths, scc_exit_prime_paths, scc_dict)
            for scc_entry_prime_path in scc_entry_prime_paths:
                yield scc_entry_prime_path

            internal_prime_paths = get_internal_prime_paths(scc_dict)
            for internal_prime_path in internal_prime_paths:
                yield internal_prime_path

# ccfg, scc_dict = get_ccfg_graph_and_scc_dict(graph)
# prime_paths_of_ccfg =  vertex_based_prime_paths_generation(ccfg, "1")
# non_empty_prime_paths = []
...
# # print(non_empty_prime_paths)
# x = cfg_complete_prime_paths_generation(non_empty_prime_paths, scc_dict, graph)
# print(x)
# y = scc_exit_prime_paths_generation(x, scc_dict)
# print(y)
# print("last")
# print(scc_entry_prime_paths_generation(x, y, scc_dict))  
# print(scc_entry_exit_paths_extraction(internal_prime_paths, v_en="2", v_ex="5"))
# print(scc_exit_paths_extraction(internal_prime_paths, v_ex="5"))
# print(scc_entry_paths_extraction(internal_prime_paths, v_en="5"))

# x = Path(["a", "b", "c"], set(), False, False)
# y = Path(["x", "y", "z"], set(), False, False)
# z = Path(["q", "r", "s"], set(), False, False)

# new_x = x.replace_node_with_path(y, 1).replace_node_with_path(z, 3)
# print("internals")
# print(get_internal_prime_paths(scc_dict))
...
#      vertex_based_prime_paths_generation(d, "B0")
#     t = time.time()
#     print(i, 2**i, t-s)
...
for prime_path in get_prime_paths(graph):
    print(prime_path)