...
def cut_outgoing_edges(digraph_manager, diblob_id: str):
    ...
    for node in outgoing_nodes:
        neighs = eval(
            f"diblob_dict{str(get_diblob_path(node)).replace(', ', '][')}['{node}']"
        )

        filtered_outgoing_nodes = [
            edge[1] for edge in outgoing_edges if edge[0] == node
        ]
        elements_to_remove = [
            {key: sorted(value)}
            for key, value in list_groupby(
                filtered_outgoing_nodes,
                {
                    node: digraph_manager[node].diblob_id
                    for node in filtered_outgoing_nodes
                },
            ).items()
        ]
...
def sort_outgoing_nodes_in_dict_repr(node_lst: list):
    ...
    """
    node_lst.sort(key=lambda x: list(x.keys())[0] if isinstance(x, dict) else x)
