...
class TestCases:
    def __init__(self, test_cases) -> None:
        self.test_case_dict = {hash(tuple(test_case)): test_case for
                               test_case in test_cases}

    @staticmethod
    def compute_edge_representation(test_case):
        return [(test_case[idx], test_case[idx+1]) for idx in range(len(test_case) - 1)]
...
class Mutation:
    ...
    def __create_hash_dict(self, mutation_method):
        ...
        for comb in combinations(test_case_mutation_space, 2):
            mutations = mutation_method(test_case_dict[comb[0]], test_case_dict[comb[1]])
            if mutations:
    ...
    def add_mutation(self, test_case1, test_case2):
       ...
       if possible_mutations:
           self.hash_dict[(hash(tuple(test_case1)), hash(tuple(test_case2)))] = possible_mutations
...
def mutation_algorithm(test_cases, cost_function, iterations=10, 
                       number_of_mutations=3, mutation_method = 'common_node_mutation', avg=0):
    mutation_set = set()
    ...
    for _ in range(iterations):
        ...
        test_cases_dict = potential_new_mutation_manager.test_case_manager.test_case_dict
        ...
        for _ in range(number_of_mutations):
            ...
            test_case_1, test_case_2 = test_cases_dict[test_1_hash], test_cases_dict[test_2_hash]
            mut_test_case_1, mut_test_case_2 = MutationMethod.get_mutated_test_cases(test_case_1, test_case_2, value)
            ...
            potential_new_mutation_manager.test_case_manager.add_test_cases(mut_test_case_1, mut_test_case_2)
            test_cases = [test_case for test_case in 
                          potential_new_mutation_manager.test_case_manager.test_case_dict.values()]
            potential_new_mutation_manager = Mutation(test_cases, mutation_method)
            test_cases_dict = potential_new_mutation_manager.test_case_manager.test_case_dict
    
        if Criterion.avg_distance(mutation_manager.test_case_manager, 
                                  potential_new_mutation_manager.test_case_manager,
                                  cost_function, avg=avg):
...
def generate_artificial_test_cases(test_length = 5, number_of_tests=4):
    ...
    for idx_1 in range(number_of_tests):
        test_case = [str(idx_2) if idx_2 % 2 == 0 else f"{idx_1}_{idx_2}" 
