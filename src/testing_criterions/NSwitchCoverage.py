...
from testing_criterions.decorators import validate_source, validate_sink, validate_reachability, validate_cost_function
...
def generate_new_cost_function(digraph_manager, cost_function):
    ...
    for edge_id in digraph_manager.edges:
        ...
        edges_x = [(x, y) for x, y in zip(edge_id_x.split('|'), edge_id_x.split('|')[1:])]
        edges_y = [(x, y) for x, y in zip(edge_id_y.split('|'), edge_id_y.split('|')[1:])]
        ...
        suffix = edges_y[len(edges_y):] if len(edges_x) > len(edges_y) \
...
def map_n_switch_digraph_to_normal_one(digraph_json):
    ...
    mapped_json = {mapping[key]: [mapping[val] for val in values]
...
def map_n_switch_cost_function_to_normal_one(n_switch_cost_function, mapping):
    return {(mapping[edge_id[0]], mapping[edge_id[1]]): cost
        ...
        for node_id in test_case:
            ...
            if node_id in reversed_mapping:
                ...
                if prev_n_switch_node_id is None:
                    ...
                    idx = 0
                    edges_x = [(x, y) for x, y in zip(prev_n_switch_node_id, prev_n_switch_node_id[1:])]
...
class NSwitchCoverage:
    ...
    def generate_n_switch_graph(digraph_manager, n_switch):
        ...
        if n_switch < 2:
            raise InvalidNSwitchException("n_switch should be at least equals to 2 or 3")
    ...
    def generate_n_switch_artifacts(self, cost_function, default_cost, n_switch):
        cost_function = EdgeCoverage.fulfill_cost_function(self.digraph_manager,
                                                           cost_function,
                                                           default_cost)
    
        n_switch_graph = self.generate_n_switch_graph(self.digraph_manager, n_switch)
        n_switch_cost_function = generate_new_cost_function(n_switch_graph, cost_function)

        mapped_json, mapping, reversed_mapping = map_n_switch_digraph_to_normal_one(n_switch_graph('B0')['B0'])
        n_switch_cost_function = map_n_switch_cost_function_to_normal_one(n_switch_cost_function, mapping)
    ...
    def get_test_cases_minimal_total_cost(self, cost_function=None,
                                          default_cost=1,
                                          n_switch=2,
                                          cost_function_multiplier=1):
        edge_coverage, reversed_mapping, n_switch_cost_function = self.generate_n_switch_artifacts(cost_function, default_cost, n_switch)

        test_cases = edge_coverage.get_test_cases_minimal_total_cost(cost_function=n_switch_cost_function, 
    ...
    @validate_cost_function()
    def get_test_cases_minimal_number_of_test_cases(self, cost_function=None, default_cost=1, n_switch=2, cost_function_multiplier=1):
        edge_coverage, reversed_mapping, n_switch_cost_function = self.generate_n_switch_artifacts(
            cost_function, default_cost, n_switch)
   
        test_cases = edge_coverage.get_test_cases_minimal_number_of_test_cases(cost_function=n_switch_cost_function,
                                                                               default_cost=default_cost,
                                                                               cost_function_multiplier=cost_function_multiplier)
        test_cases = translate_test_cases(test_cases, reversed_mapping, n_switch)
        return test_cases
    ...
    @validate_cost_function()
    def get_test_cases_set_number_of_test_cases(self, cost_function=None, default_cost=1, k=1, n_switch=2, cost_function_multiplier=1):
        edge_coverage, reversed_mapping, n_switch_cost_function = self.generate_n_switch_artifacts(cost_function, default_cost, n_switch)

        test_cases = edge_coverage.get_test_cases_set_number_of_test_cases(k=k,
